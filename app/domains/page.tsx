  ...
  return (
    <div className='md:p-20'>
      <Hero
        ...
        imgWidth={1100}
      />
      <TabBar pathname='domains' onFilledIconClick={handleFilledIconClick} onDashboardIconClick={handleDashboardIconClick} />
      <SearchBar placeholder='Search Domain by Title' />
      <div className='flex flex-wrap justify-center gap-4 md:gap-6'>
        {domains.map((domain) => (
          <DomainCard key={domain.id} name={domain.name} isAvailable={domain.available} />
        ))}
      </div>
    </div>
  );
