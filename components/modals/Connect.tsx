...
const ConnectModal: React.FC<SignUpProps> = ({ handleConnectModal, isOpen }) => {
      ...
      <AnimatePresence mode='wait'>
        ...
        {modalContent === 'getStarted' && (
          ...
          >
            <div className='flex flex-col md:px-6 gap-4 md:gap-4'>
              <h1 className={`${electronica.className} text-lg md:text-2xl`}>Welcome LAMPAS</h1>
              <span className={`${ibmSans.className} font-normal text-xs md:text-xl`} style={{ color: '#AFAFAF' }}>
                LampapuyNFT supports Solana and Solana networks. Please select a wallet provider or blockchain you want to setup with.
              </span>
            </div>
            <div className='flex flex-col md:px-4 gap-6'>
              ...
              >
                <div className='flex flex-col items-start gap-2'>
                  <span className='font-semibold text-sm text-white'>Uhmm... I’m Newbie</span>
                  <span className='font-normal text-xs' style={{ color: '#AFAFAF' }}>
                    Guide me through creating my first wallet
                  </span>
                </div>
              </button>
              <button className='w-full px-4 py-2 md:px-10 flex flex-row gap-2 items-center rounded-3xl' style={{ border: '1px solid #F5F5F5' }}>
                <Image src={phantomLogo} width={32} height={32} alt='logo' />
                <div className='flex flex-col items-start gap-2'>
                  <span className='font-semibold text-sm text-white'>Install Phantom Wallet</span>
                  <span className='font-normal text-xs' style={{ color: '#AFAFAF' }}>
              ...
              <button className='w-full px-4 py-2 md:px-10 flex flex-row gap-2 items-center rounded-3xl' style={{ border: '1px solid #F5F5F5' }}>
                <Image src={solfareLogo} width={32} height={32} alt='logo' className='rounded-lg' />
                <div className='flex flex-col gap-2 items-start'>
                  <span className='font-semibold text-sm text-white'>Install Solfare</span>
                  <span className='font-normal text-xs' style={{ color: '#AFAFAF' }}>
        ...
        {modalContent === 'newbie' && (
          <motion.div
            ...
            className='flex flex-col p-0 md:p-6 gap-6 md:gap-5'
          >
            <h1 className={`${electronica.className} text-lg md:text-2xl`}>What are Wallets?</h1>
            <span className={`${ibmSans.className} font-normal text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              1 of 3
            </span>
            <span className={`${ibmSans.className} font-normal text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              In practical terms we’re talking about a simple Chrome/Firefox extension or a mobile wallet that is protected by a unique private key. This key
              gives only you access to your wallet on your devices and should only be known to you.
            </span>
            <span className={`${ibmSans.className} font-normal flex flex-col text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              You should keep this key in a secure place. Losing or forgetting private keys means you will no longer have access to that wallet.
              <span style={{ color: '#F88430', textDecoration: 'underline' }}>Read more on Wallets and Private keys.</span>
            </span>
        ...
        {modalContent === 'settingWallet' && (
          <motion.div
            ...
            className='flex flex-col p-0 md:p-6 gap-6 md:gap-5'
          >
            <h1 className={`${electronica.className} text-lg md:text-2xl`}>Setting up a wallet</h1>
            <span className={`${ibmSans.className} font-normal text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              2 of 3
            </span>
            <span className={`${ibmSans.className} font-normal text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              Now, select a blockchain network you’d like to use on LampapuyNFT. Remember to keep your private keys and passwords safe.
            </span>
            <div className='flex flex-col gap-4'>
              <button className='w-full px-4 py-2 md:px-10 flex flex-row gap-2 items-center rounded-3xl' style={{ border: '1px solid #F5F5F5' }}>
                ...
                <div className='flex flex-col items-start gap-2'>
                  ...
                  <div className='flex flex-row gap-2 text-xs' style={{ color: '#FFB703' }}>
              ...
              <button className='w-full px-4 py-2 md:px-10 flex flex-row gap-2 items-center rounded-3xl' style={{ border: '1px solid #F5F5F5' }}>
                <Image src={solfareLogo} width={32} height={32} alt='logo' className='rounded-lg' />
                <div className='flex flex-col gap-2 items-start'>
                  <span className='font-semibold text-sm text-white'>Install Solfare Wallet</span>
                  <span className='font-normal text-xs' style={{ color: '#AFAFAF' }}>
                    Mobile and Web wallet provider for Solana
                  </span>
                  <div className='flex flex-row gap-2 text-xs' style={{ color: '#FFB703' }}>
        ...
        {modalContent === 'finalConnect' && (
          ...
          >
            <h1 className={`${electronica.className} text-lg md:text-2xl`}>You made it, great!</h1>
            <span className={`${ibmSans.className} font-normal text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              3 of 3
            </span>
            <span className={`${ibmSans.className} font-normal text-xs md:text-base`} style={{ color: '#AFAFAF' }}>
              How about you show off that new wallet of yours by connecting to LampapuyNFT first!
            </span>
            <button onClick={() => handleConnectModal()}>
              <WalletMultiButtonDynamic className='w-full'>Connect</WalletMultiButtonDynamic>
            </button>
          </motion.div>
        )}
      </AnimatePresence>
