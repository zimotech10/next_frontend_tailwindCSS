...
const CollectionCard: React.FC<Collection> = ({ name, image, coverImage, description, floor, average, isVerified, gridType }) => {
  ...
  return (
    ...
    >
      ...
      {profileImgLoading && (
        <div
          className={`absolute ${gridType === 1 ? 'top-14 md:top-[80px]' : 'top-14 md:top-32'} flex items-center justify-center`}
          style={{
        ...
        >
          <div className='loader'>{isMobile ? <SmallSpinner /> : <BigSpinner />}</div> {/* Add your spinner styling here */}
        </div>
      )}
      <img
        src={image}
