...
const FeaturedCard: React.FC<Collection> = ({ name, image, coverImage, description, floor, average, isVerified, gridType }) => {
  ...
  const coverImgWidthSize = gridType === 1 ? (isMobile ? 163 : 220) : isMobile ? 163 : 396;
  const coverImgHeightSize = gridType === 1 ? (isMobile ? 70 : 130) : isMobile ? 70 : 167;
  const profileImgSize = gridType === 1 ? (isMobile ? 34 : 40) : isMobile ? 34 : 81;
  const profileImg = gridType === 1 ? (isMobile ? '34px' : '40px') : isMobile ? '34px' : '81px';
  ...
  return (
    ...
    >
      ...
      <Image
        src={coverImage}
        alt='cover'
        width={coverImgWidthSize}
        height={coverImgHeightSize}
        style={{
          width: coverImgWidth,
          height: coverImgHeight,
